...
class CsvParserDialog(BaseWidget):
	...
	def __iter__(self): 
		if self._filename.value!=None and self._filename.value!='':
			...
			if self.xField.visible: self._cols.append(self.xColumn)
			if self.yField.visible: self._cols.append(self.yColumn)
			if self.zField.visible: self._cols.append(self.zColumn)
		else:
