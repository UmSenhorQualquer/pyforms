...
class ControlPlayer(ControlBase, QtGui.QFrame):
    ...
    def initControl(self):
        ...
        icon = QtGui.QIcon()
        pixmapOff = QtGui.qApp.style().standardPixmap(QtGui.QStyle.SP_MediaPlay)
        pixmapOn = QtGui.qApp.style().standardPixmap(QtGui.QStyle.SP_MediaPause)
        icon.addPixmap( pixmapOff, mode=QtGui.QIcon.Normal, state=QtGui.QIcon.Off )
        ...
        icon = QtGui.QIcon()
        pixmapOff = QtGui.qApp.style().standardPixmap(QtGui.QStyle.SP_DialogYesButton)
        pixmapOn = QtGui.qApp.style().standardPixmap(QtGui.QStyle.SP_DialogNoButton)
        icon.addPixmap( pixmapOff, mode=QtGui.QIcon.Normal, state=QtGui.QIcon.Off )
        icon.addPixmap( pixmapOn, mode=QtGui.QIcon.Normal, state=QtGui.QIcon.On )
    ...
    def updateFrame(self):
        ...
        if not self.videoProgress.isSliderDown():
            ...
            if self._updateVideoFrame: self.videoFrames.setValue( currentFrame )
    ...
    def value(self, value):
        ...
        if self._value and value != 0:
            ...
            self.videoProgress.setMaximum( self._value.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT) )
            self.videoFrames.setMinimum( 0 )
            self.videoFrames.setValue( 0 )
            self.videoFrames.setMaximum( self._value.get(cv2.cv.CV_CAP_PROP_FRAME_COUNT) )
